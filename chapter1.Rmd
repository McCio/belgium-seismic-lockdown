...
fitted <- fitma.list[order(aic_vals$AIC)][[2]]
...
top95pc <- pgram$spec[pgram$spec > (mean(pgram$spec) + sd(pgram$spec)*1)]
top95freq <- pgram$freq[pgram$spec > (mean(pgram$spec) + sd(pgram$spec)*1)][order(top95pc, decreasing = T)]
top95pc <- pgram$spec[pgram$spec > (mean(pgram$spec) + sd(pgram$spec)*1)][order(top95pc, decreasing = T)]
...
abline(h=mean(pgram$spec) + sd(pgram$spec)*c(.5,1:3), lty="dotted", col=c("orange", "darkgreen", "blue", "red"))
...
axis(side=4, at=mean(pgram$spec) + sd(pgram$spec) * c(.5, 1:3), labels=c(".5", "1", "2", "3"), cex.axis=0.8)
