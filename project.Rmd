...
periodgram +
  ...
  scale_y_continuous(sec.axis = sec_axis(~., breaks=mean(pgram$spec) + sd(pgram$spec) * c(1,3), labels=c("1", "2", "3")[c(1,3)], name="spectrum/sd ratio")) +
  geom_hline(yintercept=(mean(pgram$spec) + sd(pgram$spec) * c(1:3)), linetype="dotted", color = ratio.color(mean(pgram$spec) + sd(pgram$spec) * c(1:3))) +
...
trend.s <- stats::filter(seasonality.aggregate(seis.h.train), side=2, filter=weights.s)
stats::filter(seasonality.aggregate(seis.h.train), sides=2, filter = weights.s) %>% autoplot()
stats::filter(seasonality.aggregate(seis.h.train) %>% diff(), sides=2, filter = weights.s) %>% autoplot()
stats::filter(seasonality.aggregate(seis.h.train) %>% diff(lag=168), sides=2, filter = weights.s) %>% autoplot()
stats::filter(seasonality.aggregate(seis.h.train) %>% diff() %>% diff(lag=168), sides=2, filter = weights.s) %>% autoplot()
mean((seasonality.aggregate(seis.h.train) %>% diff(lag=168))[(1+(168*4)):(1+(168*6))])
