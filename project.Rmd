...
```{r periodgram}
library(spec)
pgram <- spec.pgram(build_ts(seis.h, start = seis.train.start, end = seis.train.end), main="Periodgram", detrend=F, demean=T, na.action=na.aggregate, plot=F, xlim=c(0,1/3600))
...
library(ggrepel)
periodgram +
  scale_x_continuous(sec.axis = sec_axis(~1/./3600, breaks=c(1, 6*c(1,2,4,4*7)), labels = as.integer, name="hours")) +
  scale_y_continuous(sec.axis = sec_axis(~exp(.), breaks=mean(pgram$spec) + sd(pgram$spec) * c(1:3), labels=c("1", "2", "3")[1:3], name="spectrum/sd ratio")) +
  ...
  geom_label_repel(data=data.frame(freq=top95freq, spec=top95pc)[1:3,], aes(freq, log(spec), label=round(1/freq/3600), color=ratio.color(spec)), segment.color="grey", show.legend=F) +
