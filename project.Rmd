...
```{r periodgram}
library(spec)
pgram <- spec.pgram(build_ts(seis.h, start = seis.train.start, end = seis.train.end), main="periodgram", detrend=F, demean=T, na.action=na.aggregate)
topfreq <- pgram$freq[order(pgram$spec, decreasing = T)][1:100]
topseas <- 1/topfreq
topseas.h <- topseas/3600

filter_freq <- (pgram$spec > (mean(pgram$spec) + sd(pgram$spec)*2)) 
