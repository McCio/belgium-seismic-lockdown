...
trend <- stats::filter(seis.h.ts, sides=2, filter = weights)
```
...
trend.s <- stats::filter(seis.h.ts, side=2, filter=weights.s)
...
```{r}
residuals <- seis.h.ts - trend.s - seasonality
...
```{r}
ts <- residuals+trend+seasonality
...
```{r}
acf(residuals, na.action = na.omit)
pacf(residuals, na.action = na.omit)
...
lines(c(rep(seasonality,seasons), seasonality[1:(((length(detrended)/168-seasons)*168))]), col="red")
