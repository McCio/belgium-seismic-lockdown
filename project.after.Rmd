...
print(plot_forecast(plotting.forecast) + autolayer(ts_c(`Before March '20`=seis.h.test, `From March '20`=final.testing), series="Original") + ylab("Mean hourly movement (nm)") + xlab("Weeks") + geom_vline(aes(xintercept=as.integer(as_datetime('2020-03-14T00:00:00') - seis.train.start)/7+1), linetype=2))
...
cat("80 % ",
    ...
    "% ",
    print_dec(mean(plotting.forecast$lower[1:length(seis.h.test),1] < seis.h.test & seis.h.test < plotting.forecast$upper[1:length(seis.h.test),1], na.rm = T)*100),
    "% ",
    print_dec(mean(plotting.forecast$lower[(length(seis.h.test)+1):dim(plotting.forecast$lower)[1],1] < final.testing & final.testing < plotting.forecast$upper[(length(seis.h.test)+1):dim(plotting.forecast$lower)[1],1], na.rm = T)*100),
    "%\n")
cat("95 % ",
    ...
    "% ",
    print_dec(mean(plotting.forecast$lower[1:length(seis.h.test),2] < seis.h.test & seis.h.test < plotting.forecast$upper[1:length(seis.h.test),2], na.rm = T)*100),
    "% ",
    print_dec(mean(plotting.forecast$lower[(length(seis.h.test)+1):dim(plotting.forecast$lower)[1],2] < final.testing & final.testing < plotting.forecast$upper[(length(seis.h.test)+1):dim(plotting.forecast$lower)[1],2], na.rm = T)*100),
    "%\n")
